...
export default function Upload(props) {
    ...
    const uploadFile = () => {
        ...
        console.log(sourceLang, targetLang, mode);
            axios
                .post(
                    `http://localhost:8000/translateav?source_lang=${sourceLang}&target_lang=${targetLang}&mode=${mode}`,
                    formData,
                    {
                        headers: {
                ...
                .then((response) => {
                    ...
                    // Create a download link or trigger a file download
    ...
    return (
        <div className="w-[60vw] p-6">
            <form className="rounded-lg backdrop-blur-md text-white bg-white bg-opacity-10 p-5 h-full flex flex-col items-center pt-8">
                ...
                <div className="flex flex-row w-[80%] justify-evenly items-center">
                    ...
                    >
                        {supportedDestinations.map((destination, idx) => (
                            <option key={idx} value={destination.code} className="bg-cyan">
                ...
                <div
                    className={`mb-5 w-[80%] relative ${!props.modes ? "hidden" : "visible"
                        }`}
                >
                    <div className="w-[80%] mb-5 ">
                    ...
                    <div className="flex justify-between w-full text-xs text-white mt-2 ml-0">
                        <span
                            className={`slider-label ${mode === 1 ? "opacity-100" : "opacity-50"
                                }`}
                        >
                            Best Quality
                        </span>
                        <span
                            className={`slider-label ${mode === 2 ? "opacity-100" : "opacity-50"
                                }`}
                        >
                            Quality
                        </span>
                        <span
                            className={`slider-label ${mode === 3 ? "opacity-100" : "opacity-50"
                                }`}
                        >
                            Performance
                        </span>
                        <span
                            className={`slider-label ${mode === 4 ? "opacity-100" : "opacity-50"
                                }`}
                        >
                            Best Performance
                        </span>
